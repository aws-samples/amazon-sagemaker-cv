...
def mlperf_test_early_exit(iteration, iters_per_epoch, tester, model, distributed, min_bbox_map, min_segm_map, world_size, sbridge=EmptyObject()):
...
def mlperf_training_test_early_exit(iteration, iters_per_epoch, training_ranks_comm, num_training_ranks, model, wait_this_many_iterations_before_checking_result, sbridge=EmptyObject()):
    global __eval_start_iteration
...
def terminate_evaluation_ranks(iters_per_epoch, training_ranks_comm, num_training_ranks, model, wait_this_many_iterations_before_checking_result):
    ...
    if __eval_start_iteration >= 0:
        ...
        success, _ = mlperf_training_test_early_exit(iteration, iters_per_epoch, training_ranks_comm, num_training_ranks, model, wait_this_many_iterations_before_checking_result)
        __eval_start_iteration = -1
    else:
        success = False
...
#         optimizer = FP16_Optimizer(optimizer, dynamic_loss_scale=True, dynamic_loss_scale_window=cfg.DYNAMIC_LOSS_SCALE_WINDOW)
    ...
    if rank < num_training_ranks:
        ...
        print(
                f"&&&& MLPERF METRIC THROUGHPUT={throughput} iterations / s"
        )
...
def main():
    ...
    model, success, throughput = train(cfg, rank, world_size, args.distributed, random_number_generator, seed=master_seed)

    if rank < num_training_ranks and success is not None:
        if success:
            log_end(key=constants.RUN_STOP, metadata={"status": "success", "throughput": throughput})
        else:
