...
# Hybrid loader decodes image on CPU, does transforms on GPU
_C.DATALOADER.HYBRID = False
# Use DALI input pipeline
_C.DATALOADER.DALI = False
_C.DATALOADER.DALI_ON_GPU = True
# Optionally cache evaluation images
_C.DATALOADER.CACHE_EVAL_IMAGES = False
# Always pad to dimensions suitable for cuda graphs (debug option)
_C.DATALOADER.ALWAYS_PAD_TO_MAX = True
# If > 0, pad target tensors to this size
_C.DATALOADER.MAX_ANNOTATIONS_PER_IMAGE = 0
# Use synthetic data
_C.DATALOADER.USE_SYNTHETIC_INPUT = False
...
# Add RPN head to end of backbone, thus allowing it to be cuda graphed.
_C.MODEL.BACKBONE.INCLUDE_RPN_HEAD = False
# Store scale and bias for frozen batchnorms instead of recomputing every step.
_C.MODEL.BACKBONE.DONT_RECOMPUTE_SCALE_AND_BIAS = False
...
_C.MODEL.RPN.FPN_POST_NMS_TOP_N_PER_IMAGE = False
# Custom rpn head, empty to use default conv or separable conv
_C.MODEL.RPN.RPN_HEAD = "SingleConvRPNHead"
# Use split_topk hack to improve topk throughput
_C.MODEL.RPN.USE_SPLIT_TOPK = False
# Use conv+bias+relu fusion
_C.MODEL.RPN.FUSION = True
...
_C.MODEL.RESNETS.FIRST_TRANS_FUNC = "BottleneckWithFixedBatchNorm"
...
_C.DISABLE_LOSS_LOGGING = False

_C.USE_CUDA_GRAPH = False
_C.CUDA_GRAPH_NUM_SHAPES_PER_ORIENTATION = 1
_C.DYNAMIC_LOSS_SCALE_WINDOW = 1000
_C.EVAL_SEGM_NUMPROCS = 1
_C.EVAL_MASK_VIRTUAL_PASTE = False
_C.PRECOMPUTE_RPN_CONSTANT_TENSORS = False
_C.SYNCFREE_ROI = True
_C.DEDICATED_EVALUATION_RANKS = 0
_C.DEDICATED_EVALUATION_WAIT_FOR_RESULT_ITERATIONS = (0,13,400,)

# Enable profiling
_C.ENABLE_NSYS_PROFILING = False
