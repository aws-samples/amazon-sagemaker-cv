...
__global__ void GeneratePreNMSUprightBoxesKernelBatched(
  ...
  // Going to generate pre_nms_nboxes boxes per image
      int fmap = blockIdx.z; 
      int per_image_anchor_offset = 0;
      int H_level, W_level;
      for (int i = 0; i < fmap; i++){ 
	  H_level = d_hw_array[2 * i];
	  W_level = d_hw_array[2 * i + 1];
          per_image_anchor_offset += H_level * W_level * A;
      }
      const float4 *d_anchors_offsetted = d_anchors + per_image_anchor_offset;       
      d_sorted_scores_keys += fmap * num_images * prenms_nboxes;
      d_bbox_deltas += fmap * num_images * 4*A * K_max;
      d_out_boxes += fmap * num_images * prenms_nboxes;
      d_boxes_keep_flags += fmap * num_images * prenms_nboxes;
      int H = d_hw_array[2*fmap];
      int W = d_hw_array[2*fmap + 1];
      int K = H*W;
      int KA = K*A;
    
   for (int ibox = blockIdx.x * blockDim.x + threadIdx.x; ibox < min(nboxes_to_generate,KA);
    ...
    for (int image_index = blockIdx.y * blockDim.y + threadIdx.y; image_index < num_images;
        ...
        d_anchors = d_anchors_offsetted + image_index * total_anchors_per_image;       
      ...
      //const float4 anchor = d_anchors[image_index*K_max*A +  a_idx];
      const float4 anchor = d_anchors[a_idx];
      ...
      // array first
      int deltas_idx = image_index * (K_max*A*4) + a*4*K+h*W+w;
...
std::vector<at::Tensor> GeneratePreNMSUprightBoxesBatched_cuda(
  ...
  int total_anchors_per_image = anchors.size(1);
  //const int K = H * W;
  const int num_fmaps = bbox_deltas.size(0);
  // temp Tensors
  at::Tensor boxes = at::zeros({num_fmaps, num_images, box_dim * pre_nms_nboxes}, sorted_scores.options()).to(at::kFloat);
  at::Tensor boxes_keep_flags = at::empty({num_fmaps, num_images, pre_nms_nboxes}, sorted_scores.options()).to(at::kByte);
  boxes_keep_flags.zero_();

  auto stream = at::cuda::getCurrentCUDAStream().stream();
  // Call kernel
  dim3 grid_dim(GetBlocks(pre_nms_nboxes), num_images, num_fmaps);
  GeneratePreNMSUprightBoxesKernelBatched<<<
      grid_dim,
      ...
      0, stream>>>(
          ...
          hw_array.data_ptr<long>(),
	  max_anchors,
	  total_anchors_per_image,
          K_max,
        //  K,
          A,
        //  K * A,
