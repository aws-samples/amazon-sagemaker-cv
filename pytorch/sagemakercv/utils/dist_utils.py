...
def is_sm_dist():
    ...
    if not sm_framework_params:
        return False
    sm_framework_params = json.loads(sm_framework_params)
    return bool(sm_framework_params.get('sagemaker_distributed_dataparallel_enabled', False))

def is_sm():
    """Check if running in a Sagemaker Training instance
    """
    sm_framework_params = os.environ.get('SM_FRAMEWORK_PARAMS', None)
    if sm_framework_params:
        return True
    return False
